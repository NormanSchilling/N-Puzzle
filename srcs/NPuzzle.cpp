...
NPuzzle::NPuzzle( int size, std::vector< std::vector<int> > tab ) : size( size )
{
	...
	this->puzzles.push_back( new Puzzle( size, this->puzzle_init, this->puzzle_end, 0, NULL ) );
	this->sortPuzzles.push_back( new Puzzle( size, this->puzzle_init, this->puzzle_end, 0, NULL ) );
...
void			NPuzzle::generateMove( Puzzle *puzzle, int y, int x, int y2, int x2 )
{
	...
	src[y][x] = src[y2][x2];
	src[y2][x2] = 0;
	step = new Puzzle( this->size, src, this->puzzle_end, puzzle->getRank() + 1, new Puzzle( *puzzle ) );
